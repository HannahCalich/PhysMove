...
FitDist <- function (Displacements, dist=c("pl","exp","lnorm"), set_xmin=NULL, Full=FALSE, Normalize=TRUE) {
  ...
  #if people dont type in "pl","exp","lnorm" snd wrning
  ...
  if ("lnorm" %in% dist){
    ...
    if (Full==FALSE){
      if (missing(set_xmin)){
        ...
        for (i in 1:(length(xmins)-1)){ #swamped this line in because i think the is finite code above will help with this issue
