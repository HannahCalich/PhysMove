...
LinkList <- function(species_df, grid=4, hours=24, range_hr=6){
  ...
  # Create new dataframe with all destination cells re-numbered in order of visit
    ...
    colnames(ordered.weights)<-c("origin", "destination", "weight") # rename columns following infomap requirements
    assign("LinkListOutput", ordered.weights, envir = .GlobalEnv)
    assign("grid",grid, envir = .GlobalEnv)
    message("To identify infomap communities please export LinkList as .txt file and upload .txt file at https://www.mapequation.org/infomap/#Install Important settings to consider when configuring analysis: '-i link-list -k --tree -d'. '-i link-list' identifies the input at a link list, '-k' enables self-links, '--tree' ensures the output is in .tree format, and '-d' assumed directed links.")
}
