...
### truncated exponential dist to add to beginning of powerlaw sample data range of 0.001 to 0.25km
{ set.seed(1)
exp_samplesize <- 50
lambda <- 0.125
distmin <- 1.0e-3 #in km
distmax <- 2.4e-1
exp_x <-RGeode::rexptr(exp_samplesize, lambda, c(distmin, distmax))
angle <- runif(exp_samplesize,0,360) #random angles between 0-360
Expsample<-data.frame("ref"=c(1),"lon"=c(rep(0,exp_samplesize)),"lat"=c(rep(0,exp_samplesize)),"day"=as.POSIXct("2015-01-01 09:30:31")) # 1 track
Radius <- 6371 #Earth Radius in km (disp are in km)
rad <- 3.141592653589793/180 #Python has more digits of pi than R, so value pasted here instead of "pi"
for (i in 1:(exp_samplesize-1)){
  d <- exp_x[i]
  a <- angle[i]*(rad)
...
angle <- runif(samplesize,0,360) #random angles between 0-360
PLsample<-data.frame("ref"=c(1),"lon"=c(rep(0,samplesize)),"lat"=c(rep(0,samplesize)),"day"=as.POSIXct("2015-01-01 09:30:31")) # 1 track
PLsample$lon[1]<-Expsample$lon[exp_samplesize]
PLsample$lat[1]<-Expsample$lat[exp_samplesize]
PLsample$day[1]<-Expsample$day[exp_samplesize]
head(PLsample)
tail(Expsample)
Radius <- 6371 #Earth Radius in km (disp are in km)
rad <- 3.141592653589793/180 #Python has more digits of pi than R, so value pasted here instead of "pi"
...
plTest<-rbind(Expsample,PLsample[2:samplesize,])
# plTest<-rbind(Expsample,PLsample)
plot(plTest$lon,plTest$lat,type="l", lty=1)
summary(plTest)
summary(PLsample)
} ## not working
############################################################################################################################################################
...
### THIS VERSION WAS NOT USED IN FINAL DATASET ### -- USE THE rlnormTrunc version
# set.seed(1)
# samplesize<-1000
# sigma <- 2
# r <- runif(samplesize,0,1) #uniform random number between 0-1
...
plot(LNsample$lon,LNsample$lat,type="l", lty=1)
lnormSample<-LNsample
rm(list=ls()[! ls() %in% c("lnormSample")])
} ## rlnormTrunc and rlnorm methods ## USE THIS VERSION
...
setwd("Y:/Git/PhysMove/data")
load("plSample.RData")
load("expSample.RData")
load("lnormSample.RData")

data<-plSample
data<-expSample
data<-lnormSample

CalcDisp(data, max_hr = 24)
FitDist(Displacements, Normalize = FALSE, Full=TRUE)
