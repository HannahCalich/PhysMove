...
**Figure S2** Scatter plot of mean (grey points; q=1) and root-mean-square (RMS; black points; q=2) displacements (d) in kilometres (km) from tracks dataset over time (T) in days, fit to a linear model (red line with standard error shaded in grey). Plot created with `RMS()` default parameters. 
...
(Figure S3 - Figure S4; Figure 1 in main text). The normalised parameter allows the data to be normalised before plotting, 
...
understanding of displacement patterns (see Figure S5).
...
`FitDist()` is used to identify the best-fit d_min for each distribution (Figure S 12). 
...
- `legend`: add a legend (`legend=TRUE`, by default).n
