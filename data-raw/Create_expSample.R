...
tracks <- 5 #how many tracks do you want to make?
...
expSample <- data.frame("ref"=0,"lon"=startLonRange[1],"lat"=startLatRange[1],"day"=dates[1]) # start loop, delete row at end of loop
...
for (i in 1:tracks){
  ...
  expSample<-rbind(expSample,c(data.frame("ref"=rep(i,samplesize[i]),"lon"=rep(startLonRange[i],samplesize[i]),"lat"=rep(startLatRange[i],samplesize[i]),
  ...
  for (j in 1:(samplesize[i]-1)){ #for each new location in each track  1-411
    ...
    lat1 <- expSample$lat[startRow+j]*(rad) # convert to radians - current lat
    long1 <- expSample$lon[startRow+j]*(rad) # convert to radians - current long
    ...
    expSample$lon[startRow+j+1] <- long2/rad # convert to degrees and save
    expSample$lat[startRow+j+1] <- lat2/rad # convert to degrees and save
    expSample$day[startRow+j+1] <- expSample$day[startRow+j]+(24*60*60) # previous day plus number of hours it would take the animal to travel that distance
  }
}
expSample<-expSample[-1,]
cols <- data.frame(
  col = palette.colors(length(unique(expSample$ref))),
  id = unique(expSample$ref)
)
with(expSample, {
  plot(lon,lat,pch = 16, col = cols$col[match(ref, cols$id)], type = "p", ylim=c(min(lat)-0.25,max(lat)+0.25),xlim=c(min(lon)-0.25,max(lon)+0.25),cex=0.5)
  for (l in unique(ref)) {
