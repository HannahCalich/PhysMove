...
set.seed(1)
samplesize <- floor(stats::runif(n.tracks,200,1000)) # how many locations is the track going to have
...
for (i in 1:n.tracks){ # for each track
  ...
  # calculate angles
  set.seed(1)
  directedForwardAngle1 <- round(stats::runif((ceiling((directedforward/2)*samplesize[i])),330,360),2)
  set.seed(1)
  directedForwardAngle2 <- round(stats::runif((ceiling((directedforward/2)*samplesize[i])),0,30),2)
  set.seed(1)
  directedBackwardAngle <- round(stats::runif((ceiling((directedbackward)*samplesize[i])),150,210),2)
  set.seed(1)
  randomAngle <- stats::runif(ceiling((random*samplesize[i])),0,360)
...
# Plot tracks
cols <- data.frame(
  col = grDevices::rainbow(length(unique(tracks$ref))),
...
with(tracks, {
  ...
  for (l in unique(ref)) {
    ...
    graphics::lines(lon[ref == l], lat[ref == l], type = "l", col = cols$col[match(l, cols$id)])
  }
})
