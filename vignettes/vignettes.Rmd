...
This is a brief vignette to accompany the PhysMove R package. Here, we demonstrate how PhysMove can be used to calculate each of the methods discussed in the main text and review all relevant functions and parameters. We demonstrate each function with a simulated telemetry dataset, called `tracks`, which is automatically loaded with PhysMove.This vignette also includes all of the codes required to replicate each of the results presented in the main text.

# Install

We recommend users install PhysMove through the devtools R package. The
code below will install devtools and PhysMove and load the PhysMove R package.
...
```{r install physmove, echo=TRUE, message=FALSE, highlight=TRUE, clipboard=TRUE}
# Download PhysMove 
...
As described in the main text, `tracks` is a simulated telemetry dataset containing 25 unique tracks with a defined set of movement parameters.
`Tracks` was created using a biased, uncorrelated random walk model with variable step lengths drawn from an exponential distribution with λ = 0.125. We defined the turning angles such that 30% indicated directed forward movement (movements with angles <30° or >330°), and 30% indicated directed return movement (angles between 150-210°), allowing the remainder (40%) to be randomly between 0-360°.These metrics were chosen because they are broadly consistent with literature interpretations of animals moving through a resource-rich habitat. See main text for further details. The code used to make the `tracks` dataset is available in the PhysMove `doc` folder on GitHub as “CreateTracks.R”.
...
A map of telemetry data can be created using our `PlotTracks()` function (Figure V1). 
`PlotTracks()` has three optional parameters:
...
calculating mean and root-mean-square (RMS) displacements and
plotting them over time (Figure V2). 

`RMS()` includes four optional parameters:
...
(`wBins=1.1`, by default), 
  ...
  * `lm`: fit a linear model to examine the relationship between the root-mean-square 
  displacement values and time (`lm=TRUE`, by default). 

`RMS()` results are output as a list. The first list element is a dataframe of results with three columns: 
  * *timeWindow*: binned time windows in days (or whatever unit was set
using `timeUnit`) 
  * *meanDisplacement*: mean displacement values in km that correspond with *timeWindow*
  * *rmsDisplacement*: root-mean-square displacement values that correspond with *timeWindow*

The second list element is a dataframe of results from
the linear model and is only exported when `lm=TRUE`. The slope of the linear model is used to describe the scale of
movement (see Table 3 in the main text for suggestions on interpreting your results).
...
location estimates separated by 10 time windows: 24 ± 6 hours, 48 ± 6 hours, 72 ± 6 hours, etc., until 240 ± 6 hours. 
...
window and so on. For example, by default, the first list element would include displacements calculated over 24 ± 6 hours, and the 10th list element
would contain displacements calculated over 240 ± 6 hours.
...
### Probability density functions (pdf) of displacements

Probability density functions (pdf) of displacements describe the probability distribution 
of the displacements and can be used to calculate the probability of displacements occurring
(Figure V3 - Figure V4; Figure 1 in main text). See main text for further information.

The `PlotDispPDF()` function calculates pdfs of displacements using 3 optional parameters:
...
**Figure V3** Probability density function (pdf) plot of displacements from the `tracks` dataset calculated over 10 time windows, 24 to 240 hours at 24 ± 6-hour time intervals with `CalcDisp()`. Plot created with `PlotDispPDF()` where `normalised=FALSE`.
...
**Figure V4** Probability density function (pdf) plot of normalised displacements from the `tracks` dataset calculated over 10 time windows, 24 to 240 hours at 24 ± 6-hour time intervals with `CalcDisp()`. Plot created with `PlotDispPDF()` default parameters. 

## Search patterns 

After displacements are calculated PhysMove functions can be used to identify the best-fit
...
(Figure 1 in main text, Figure V5 for a methodological overview): 
...
truncated distributions)
  ...
  * `dist`: distributions you want to fit to the displacement
  data. `FitDist()` can fit continuous power-law (“pl”), exponential
  (“exp”), and lognormal (“lnorm”) distributions
  (`dist=c("pl","exp","lnorm")`, by default). To fit only one or two
  distributions simply remove the distributions you are not interested in running
  e.g., `dist=c("exp","lnorm")`.
  * `full`: determines if distributions are fit over
  ...
  * `set_dmin`: used to either force the algorithm to automatically identify the best-fit d_min for each
  ...
  distribution fit (e.g., `set_dmin=1`). Note that set_dmin can only be applied if `full = FALSE'.
...
`FitDist()` outputs a list including two list elements. The first list element is a dataframe that includes: 
  ...
  * *nTail*: the number of values greater than or equal to dmin.
  ...
  The second list element is a logical argument that records if the displacements were normalized (TRUE), or not (FALSE). This is output
  because it is requires for `PlotDist()` and `CompDist()`. 
...
identifies the best-fit distribution for the displacements. Note that
...
![](https://user-images.githubusercontent.com/73092681/223385586-b06b3dd6-d86a-4eeb-ae26-54484e6a4c28.png){width=70%}
...
```{r find best-fit d_min for each dist , echo=TRUE}
...
```{r compdist pl, echo=TRUE, message=FALSE}
...
```{r compdist exp, echo=TRUE, message=FALSE}
...
```{r compdist lnorm, echo=TRUE, message=FALSE}
