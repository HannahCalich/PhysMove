...
kilometres over set time windows. 
...
truncated distributions)
  ...
  The second list element is a logical argument that records if the displacements were normalized (TRUE), or not (FALSE). This is output because   it is required for `PlotDist()` and `CompDist()`. 

2. `PlotDist()`: uses the results from `FitDist()` to plot ccdfs of the displacements with fit lines for each distribution (e.g., Figure V6).
  ...
  `PlotDist()` outputs a plot and a dataframe of the values used in the plot.
...
range of data (e.g., when `full=TRUE` or if `set_dmin≠NULL`).
  
  `CompDist()` includes one optional parameter: 
...
We begin by calculating displacements over 24 ± 6 hours with `CalcDisp()` and plotting a pdf of the displacements with `PlotDispPDF()` (Figure V6; Figure 1 in main text). 
