...
In addition to `CheckTracks()`, you can also compare your dataframe to the `tracks` dataset to ensure your data are formatted correctly.
...
The code used to make the `tracks` dataset is available in the PhysMove doc folder on GitHub as “CreateTracks.R”.

```{r plot tracks, out.width="60%"}
PlotTracks(tracks)
```
...
plotting them over time (Figure S2). `RMS()` includes four optional parameters:
...
`RMS()` outputs a list object in three columns: 
...
Probability density functions (pdf) can be created....####
...
**Figure S3** Probability density function (pdf) plot of normalised displacements from the tracks dataset calculated over 10 time windows, 24 to 240 hours at 24 ± 6-hour time intervals with `CalcDisp()`. Plot created with `PlotDispPDF()` default parameters. 

## INTRODUCE PDF WITH NORMALIZE = FALSE
...
and *nTail*, the number of values greater than or equal to dmin.
