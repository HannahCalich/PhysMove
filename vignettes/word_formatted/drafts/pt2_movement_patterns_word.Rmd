...
distributions) or to displacements truncated by a minimum value (i.e., truncated distributions). `fitDist()` requires a list of values (e.g., displacements calculated using `calcDisp()`) and includes four optional parameters:

  * `dist`: Continuous distributions that will be fit to the data. Possible values are power law ("pl"), exponential ("exp"), or log-normal ("lnorm") continuous distributions (`dist=c("pl","exp","lnorm")`, by default). To fit only one or two distributions simply remove the distribution(s) you are not interested in running, e.g., `dist=c("exp","lnorm")`.
...
Uses the results from `fitDist()` to plot ccdfs of the displacements with fit lines for each distribution. `plotDist()` requires a list of values (e.g., displacements calculated using `calcDisp()`), and results from `fitDist()`, and includes five optional parameters:
...
`compDist()` can only be used when all distributions are fit to the same range of data (e.g., when `full=TRUE` or if `set_xmin≠NULL`). See Figure V5 for a methods overview. `compDist()` requires a list of a list of values (e.g., displacements calculated using `calcDisp()`), the results from `fitDist()`, and includes one optional parameter:
