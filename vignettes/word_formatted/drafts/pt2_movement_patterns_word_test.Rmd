...
```{r summarise rms results, eval=TRUE}
...
```{r calculate hurst exponent, eval=TRUE}
...
```{r sum disp from 1st time window, eval=TRUE}
...
```{r plot all norm disp, eval=TRUE}
...
```{r plot all disp (not norm), eval=TRUE}
...
```{r calc disp over 24 hours, eval=TRUE}
...
```{r find best-fit xmin for each dist, eval=TRUE}
...
```{r fit dist with lnorm, eval=TRUE}
...
```{r view random results, eval=TRUE}
# Summarise RMS results
...
```{r lm of randomised results, echo=TRUE, eval=TRUE}
...
```{r slope of randomised results, echo=TRUE, eval=TRUE}
...
```{r plot random tracks, eval=TRUE}
...
```{r summarise turning angles, eval=TRUE}
...
```{r plot angles with a circle plot, eval=TRUE}
